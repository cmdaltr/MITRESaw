...
def write_csv_summary(
    ...
    # Entry format (after extract_indicators appends evidence):
    ...
    with open(csv_path, "a", newline="") as opmitre_csv:
        ...
        for dataset in consolidated_techniques:
            ...
            writer.writerow([
                ...
                parts[12] if len(parts) > 12 else "",  # evidence_type
                parts[13] if len(parts) > 13 else "",  # evidence_indicators
            ])
            if queries:
                query_pairings.append(
                    "{}||{}||{}".format(
                        parts[2], parts[3],
                        parts[13].replace("\\\\\\\\", "\\\\").lower() if len(parts) > 13 else "",
                    )
                )
